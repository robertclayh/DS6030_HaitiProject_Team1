...
# Assume that 'train_data_grid_random' is a data frame that includes:
# - A column “Class” (the grouping variable)
# - Grid coordinates “x” and “y” (generated as described previously)
# - Predictor variables: Red, Green, Blue, Luminance, a, b, Hue, Saturation, Value, Red_Prop, Green_Prop, Blue_Prop, Dispersion, Hue_Shifted
...
# Apply the neighborhood function to each row of the training data (randomly ordered grid)
# and append the 3×3 neighborhood means as new columns.
neighbor_means_df <- train_data_grid_random %>%
  rowwise() %>%
  mutate(neighbor_values = list(compute_neighbor_means(train_data_grid_random, Class, x, y))) %>%
  ungroup() %>%
  unnest(cols = c(neighbor_values))

# Inspect the updated data frame (showing the new neighborhood mean features)
print(head(neighbor_means_df))
```
