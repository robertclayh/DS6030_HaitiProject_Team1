...
#| fig.height: 12
#| out.width: 80%
#| fig.cap: Metrics as a function of threshold optimization in the RGB color space.
#| warning: FALSE
...
#| fig.height: 12
#| out.width: 80%
#| fig.cap: Metrics as a function of threshold optimization in the CEILab color space.
#| warning: FALSE
...
#| fig.height: 12
#| out.width: 80%
#| fig.cap: Metrics as a function of threshold optimization in the HSV color space.
#| warning: FALSE
...
combined_cm <- logreg_cm_rgb + logreg_cm_lab +
...
```{r}
# Function to predict class labels at optimal threshold
...
# Apply model predictions to holdout data
...
logreg_rgb_vect <- vect(logreg_rgb_predictions, geom = c("Lon", "Lat"), crs = "EPSG:4326")
logreg_lab_vect <- vect(logreg_lab_predictions, geom = c("Lon", "Lat"), crs = "EPSG:4326")

# Reproject to UTM (for consistent spatial operations)
...
```{r}
# Create empty raster
raster_template <- rast(ext(logreg_rgb_utm), resolution = 0.1, crs = "EPSG:32618")
...
# Apply mode filter
smoothed_rgb <- focal(r_rgb, w = window_size, fun = modal, na.policy = "omit", na.rm = TRUE)
smoothed_lab <- focal(r_lab, w = window_s
                      ize, fun = modal, na.policy = "omit", na.rm = TRUE)
```
...
visualize_conf_mat_smoothed <- function(smoothed_df, model_name, color_space) {
  ...
  autoplot(cm, type = "heatmap") +
    labs(title = sprintf("%s (%s) - Smoothed", model_name, color_space))
}
...
combined_cm_smoothed <- logreg_cm_rgb_smoothed + logreg_cm_lab_smoothed +
  plot_annotation(title = "Holdout Set Confusion Matrices for Smoothed Predictions (RGB and CIELab)")
